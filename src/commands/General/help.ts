...
module.exports = class HelpCommand extends Command {
  ...
  async run(message: Message, args: Args) {
	...
	if (interaction.user.id != message.author.id) {
	  const embed =
	      new BediEmbed().setTitle('Help Reply').setColor(colors.ERROR).setDescription('You did not run this command');
	...
	await helpMessage.edit({
      ...
      const embed = new BediEmbed().setTitle(`Help Reply - ${prefix}${command.name} command`).setDescription(command.description);
	...
	embed.addField('Detailed Description', 'Usage: `' + prefix + command.detailedDescription, false);
      ...
      for (const alias of command.aliases) { aliasString += `${Formatters.inlineCode(`${prefix}${alias}`)} `; }
