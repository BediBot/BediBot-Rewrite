...
module.exports = class GetQuotesCommand extends Command {
        ...
        async run(message: Message, args: Args) {
                ...
                const embed =
                    new BediEmbed().setTitle('Get Quotes Reply').setColor(colors.ACTION).setDescription('Searching for Quotes');
                ...
                for (let i = 0; i < quotes.length; i += MAX_QUOTES_PER_PAGE) {
                        ...
                        for (let j = 0; j < MAX_QUOTES_PER_PAGE; j++) {
                                ...
                                if (quotes[i + j].date)
                                        embed.addField(
                                            `<t:${Math.round(quotes[i + j].date.valueOf() / 1000)}:f>`, quoteText, false);
                                else
