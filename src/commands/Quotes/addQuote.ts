...
module.exports = class AddQuoteCommand extends Command {
  ...
  async run(message: Message, args: Args) {
    ...
    if (quote.length === 0) {
      const embed =
	  new BediEmbed().setColor(colors.ERROR).setTitle('Add Quote Reply').setDescription('You cannot submit an empty quote.');
      return message.reply({embeds: [embed]});
    }
    ...
    const embed =
	new BediEmbed().setColor(colors.ACTION).setTitle(`${TITLE_BEFORE_NUM_APPROVALS}0/${settingsData.quoteApprovalsRequired}`);
	...
	const embed =
	    new BediEmbed().setTitle(`Add Quote Reply - Approvals: ${numApprovals}/${settingsData.quoteApprovalsRequired}`);
