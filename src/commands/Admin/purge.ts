  ...
  constructor(context: PieceContext) {
    super(context, {
      ...
      detailedDescription: 'purge <integer>`' +
          `\nThe number represents the number of messages to purge. Maximum: ${Formatters.inlineCode(BULK_DELETE_MAX.toString())}`,
    });
  }
  ...
  async run(message: Message, args: Args) {
    ...
    */
    ...
    const embed = new BediEmbed()
        ...
        .setDescription(
            `Successfully purged ${Formatters.inlineCode(
                numMessagesDeleted.toString())} messages in ${message.channel.toString()} from ${Formatters.inlineCode(message.guild?.name!)}`);
