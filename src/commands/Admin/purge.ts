...
module.exports = class PurgeCommand extends Command {
  ...
  async run(message: Message, args: Args) {
    ...
    /* //Commented out code to support purging messages from a specific user, rationale is because the current interface is
    unintutive if(!args.finished)
    {
	...
	const number_of_msgs_deleted = await purge_messages_from_specific_user(message, number_of_msgs_to_delete.value,
    user.value.id); if(number_of_msgs_deleted == 0)
	{
	    const embed = new BediEmbed()
		.setColor(colors.ERROR)
		.setTitle('Purge Reply')
	...
	else
	{
		...
		const embed = new BediEmbed()
		.setTitle('Purge Reply')
		.setDescription('Successfully purged `' + number_of_msgs_deleted + "` messages from `" + user.value.tag +"` in `"
    + message.guild?.name + "`")
