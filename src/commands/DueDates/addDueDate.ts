...
module.exports = class AddDueDateCommand extends Command {
        ...
        async run(message: Message, args: Args) {
                ...
                if (!title.success || !month.success || !day.success || !year.success)
                        return invalidSyntaxReply(message, settingsData);
                ...
                const categorySelectMenu =
                    new MessageSelectMenu().setCustomId('categorySelect').setPlaceholder('Select a Category');
                ...
                if (dateOnly)
                        embed.setDescription(
                            `${Formatters.inlineCode(title.value)} to be due <t:${Math.round(date.valueOf() / 1000)}:D>`);
                else
                        embed.setDescription(
                            `${Formatters.inlineCode(title.value)} to be due <t:${Math.round(date.valueOf() / 1000)}:F>`);
