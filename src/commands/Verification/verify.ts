...
module.exports = class VerifyCommand extends Command {
        ...
        async run(message: Message, args: Args) {
                ...
                if (existingEmailHash) {
                        ...
                        const embed =
                            new BediEmbed().setTitle('Verify Reply').setDescription('You have been automatically verified!');
                ...
                if (!emailAddress.success) {
                        const embed = new BediEmbed()
                                          ...
                                          .setDescription(
                                              ...
                                              settingsData.prefix + 'verify <emailAddress>`');
                        return message.channel.send({embeds: [embed]});
                }
                ...
                if (!(isEmailValid(emailAddress.value) && emailAddress.value.endsWith(settingsData.emailDomain))) {
                        const embed = new BediEmbed()
                                          ...
                                          .setTitle('Verify Reply')
                                          .setDescription(
                                              'Invalid Syntax!\n\nMake sure your email is in the format `email@' +
                                              settingsData.emailDomain + '`');
                        return message.channel.send({embeds: [embed]});
                }
                ...
                await addPendingVerificationUser(
                    author.id, guildId as string,
                    await hashString(emailAddress.value.substring(0, emailAddress.value.indexOf('@'))), uniqueKey);
