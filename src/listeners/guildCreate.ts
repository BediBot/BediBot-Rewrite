...
module.exports = class GuildCreateListener extends Listener {
  ...
  public async run(guild: Guild) {
	...
	{_id: guild.id});  // This will delete existing settings data
	...
	new BediEmbed()
		...
		`Run ${
		    Formatters.inlineCode(
			settingsData.prefix +
			'settings')} to see the various settings that can be changed.`)
	    .addField(
		'Due Dates',
...
Then, run ${
		    Formatters.inlineCode(
			settingsData.prefix +
			'displayDueDates <category>')} to display them in a channel.`)
	    .addField(
		'Verification',
		`To set up verification, first set up the email domain and verified role (see ${
			...
			Formatters.inlineCode(
			    settingsData.prefix +
			    'toggleModules')} to enable verification.`)
	    .addField(
		...
		`Morning announcements will send a morning message with a random quote from your guild at a set time.\n` +
			...
			Formatters.inlineCode(
			    settingsData.prefix +
			    'morningAnnouncement <time>')} in a channel to set them up.`)
	    .addField(
